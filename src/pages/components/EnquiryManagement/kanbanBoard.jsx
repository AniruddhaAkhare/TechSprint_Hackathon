...
// //   const handleOpenConflictingEnquiry = (conflictingEnquiry) => {
// //     setSelectedEnquiry(conflictingEnquiry);
...
// //           } else {
// //             // Log unexpected stages for debugging
// //             console.warn(`Enquiry ${enquiry.id} has invalid stage: ${enquiry.stage}`);
// //             updatedColumns["pre-qualified"].items.push({
// //               ...enquiry,
...
// //           totalAmount: sourceItems.reduce((sum, item) => sum + (Number(item.amount) || 0), 0),
// //         },
...
// //         const historyEntry = {
// //           action: `Changed stage to ${stageDisplay[destination.droppableId]?.name || destination.droppableId}`,
// //           performedBy: currentUser?.displayName || currentUser?.email || "Unknown User",
// //           timestamp: new Date().toISOString(),
// //         };
...
// //     .filter((enquiry) => filteredEnquiries([enquiry]).length > 0);
...
// import { DragDropContext } from "@hello-pangea/dnd";
// import { db, auth } from "../../../config/firebase";
// import { collection, onSnapshot, updateDoc, doc, addDoc, deleteDoc, query, where, or } from "firebase/firestore";
// import { FaSearch, FaCircle, FaCheckCircle, FaTimesCircle, FaClock, FaTrash, FaEdit } from "react-icons/fa";
// import { format } from "date-fns";
// import * as XLSX from "xlsx";
// import FiltersDropdown from "./FiltersDropdown";
...
// import TagsModal from "./TagsModal";
// import Column from "./Column";
// import { toast } from "react-toastify"; // Added for user-friendly error messages
...
//   const handleViewEnquiry = (enquiry) => {
//     if (!canView) {
//       toast.error("You don't have permission to view enquiry details.");
//       return;
//     }
//     setSelectedEnquiry(enquiry);
//     setIsNotesMode(false);
...
//       top = rect.top + scrollY - popupHeight - 5;
//     }

//     setNotePopupPosition({ top, left });
//     setIsTypePopupOpen(true);
//   };
...
//     }
//     if (!selectedEnquiry) {
//       toast.error("No enquiry selected to add note.");
//       return;
//     }
//     if (!canUpdate) {
...
//       // Branches listener
//       unsubscribes.push(
//         onSnapshot(collection(db, "instituteSetup", "9z6G6BLzfDScI0mzMOlB", "Center"), (snapshot) => {
//           const branchesData = snapshot.docs.map((doc) => ({ id: doc.id, name: doc.data().name }));
//           setBranches(branchesData);
//         })
//       );
...
//       // Enquiries listener with user-specific filter
//       const userIdentifier = currentUser.displayName || currentUser.email;
//       const enquiriesQuery = query(
//         collection(db, "enquiries"),
//         or(
//           where("createdBy", "==", userIdentifier),
//           where("assignTo", "==", userIdentifier)
//         )
//       );
...
//   const filteredEnquiries = useCallback((items) => {
//     if (!canView) return [];
//     return items.filter((enquiry) => {
//       const matchesSearch =
//         !searchTerm ||
...
//       return sortConfig.direction === "asc" ? aDate - bDate : bDate - aDate;
//     });
//   }, [filters, searchTerm, sortConfig, canView]);

//   const handleSort = (key) => {
//     if (!canView) {
//       toast.error("You don't have permission to sort enquiries.");
//       return;
//     }
//     setSortConfig((prev) => ({
//       key,
...
//   const onDragEnd = async (result) => {
//     if (!canUpdate) {
//       toast.error("You don't have permission to update enquiry stages.");
//       return;
//     }
//     const { source, destination } = result;
//     if (!destination) return;
...
//   const handleAddEnquiryClick = () => {
//     if (!canCreate) {
//       toast.error("You don't have permission to create enquiries.");
//       return;
//     }
//     setIsEnquiryTypeModalOpen(true);
//   };
...
//           lastTouched: timestamp,
//           createdBy: userName,
//           assignTo: userName, // Assign to the creator by default
//           history: [
//             {
//               action: "Enquiry created",
//               performedBy: userName,
...
//       setBulkEnquiries([{ fullName: "", phoneNumber: "", email: "", college: "" }]);
//       setValidationErrors([]);
//       toast.success("Bulk enquiries added successfully!");
//     } catch (error) {
//       console.error("Error adding bulk enquiries:", error);
//       toast.error(`Failed to add bulk enquiries: ${error.message}`);
//     }
//   };
...
//     }
//     if (bulkEnquiries.length === 1) {
//       toast.error("At least one enquiry is required.");
//       return;
//     }
//     const updatedEnquiries = bulkEnquiries.filter((_, i) => i !== index);
//     setBulkEnquiries(updatedEnquiries);
...
//   const handleSelectForAction = (enquiryId) => {
//     if (!canUpdate && !canDelete) {
//       toast.error("You don't have permission to select enquiries for update or deletion.");
//       return;
//     }
//     setSelectedEnquiries((prev) =>
//       prev.includes(enquiryId)
...
//     if (selectedEnquiries.length === 0) {
//       toast.error("No enquiries selected for deletion.");
//       return;
//     }
...
//     } catch (error) {
//       console.error("Error deleting enquiries:", error);
//       toast.error(`Failed to delete enquiries: ${error.message}`);
//     }
//   };
...
//     }
//     if (selectedEnquiries.length === 0) {
//       toast.error("No enquiries selected for update.");
//       return;
//     }
//     setMassUpdateData({ assignTo: "", tagsToAdd: [], tagsToRemove: [] });
//     setIsMassUpdateModalOpen(true);
...
//       setSelectedEnquiries([]);
//       setMassUpdateData({ assignTo: "", tagsToAdd: [], tagsToRemove: [] });
//       toast.success("Selected enquiries updated successfully!");
//     } catch (error) {
//       console.error("Error updating enquiries:", error);
//       toast.error(`Failed to update enquiries: ${error.message}`);
//     }
//   };
...
//           <div>
//             <h1 className="text-xl sm:text-2xl font-semibold">Enquiry Management</h1>
//             <p className="text-gray-500 text-sm sm:text-base">Manage and track your enquiries.</p>
//           </div>
//           <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto relative">
//             {canUpdate && (
//               <button
...
//                   No enquiries found. You may not have created or been assigned any enquiries yet.
//                 </div>
//               ) : (
//                 filteredEnquiries(allEnquiries).map((item) => (
//                   <div
