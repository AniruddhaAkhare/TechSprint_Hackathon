...
export default function Instructor() {
  ...
  const [instructorCode, setInstructorCode] = useState("+91");
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        try {
            ...
            setInstructorList(
                instructors.docs.map((doc) => ({ id: doc.id, ...doc.data() }))
            );

            // Fetch institute and centers
            ...
            if (!instituteSnapshot.empty) {
                ...
                setCenters(
                    centerData.docs.map((doc) => ({ id: doc.id, ...doc.data() }))
                );
            }
    ...
    fetchData();
      ...
      const handleDeleteInstructor = async () => {
        if (!canDelete || !selectedInstructor?.id) return; // Prevent deletion if no permission
        try {
