...
const countryCodes = [
  ...
  { code: "+298", label: "Faroe Islands (+298)" }, // Fixed typo: "Far deviance" → "Faroe Islands"
...
const BranchFormModal = ({
  ...
  const extractPhoneDetails = (contactNumber) => {
    ...
    const number = match ? contactNumber.replace(/^\+\d+/, "").trim() : contactNumber;
  ...
  const [branchForm, setBranchForm] = useState({
    ...
    // contactNumber: initialNumber, // Store only the number part
  ...
  const handleBranchSubmit = async (e) => {
    ...
    if (branchForm.contactNumber && branchForm.contactNumber.length !== 10) {
    ...
    try {
      ...
      const fullContactNumber = branchForm.contactNumber
        ? `${branchCountryCode}${branchForm.contactNumber}`
        : "";

      const updatedBranchForm = {
        // istituteId, // Include instituteId in the document
      ...
      if (currentBranch && canUpdate) {
        // Update existing branch in Branch collection
        const branchRef = doc(db, "Branch", currentBranch.id);
        ...
        // Add new branch to Branch collection
        const newBranch = await addDoc(collection(db, "Branch"), updatedBranchForm);
        setBranches((prev) => [
          ...prev,
