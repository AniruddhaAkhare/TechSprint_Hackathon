...
import { PutObjectCommand } from '@aws-sdk/client-s3';
import AddMCQModal from './AddMCQModal.jsx';
import QuestionBankModal from '../../QuestionBank/QuestionBank.jsx'; // New component
...
const AddMaterial = () => {
  ...
  const [feedbackText, setFeedbackText] = useState('');
  const [isMCQModalOpen, setIsMCQModalOpen] = useState(false);
  const [isQuestionBankOpen, setIsQuestionBankOpen] = useState(false); // New state for QuestionBankModal
  const [uploadError, setUploadError] = useState(null);
  const [state, setState] = useState('draft');
  const [scheduledAt, setScheduledAt] = useState('');
  const navigate = useNavigate();
  ...
  const handleAddQuiz = async (selectedQuestions) => {
    try {
      ...
      await addDoc(collection(db, collectionPath), {
        type: 'quiz',
        questions: selectedQuestions, // Store selected questions
        state,
        scheduledAt: state === 'scheduled' ? new Date(scheduledAt).toISOString() : null,
        createdAt: new Date(),
      });
    ...
    <div className="flex flex-col w-full p-4 md:ml-80">
      ...
      <div className="space-y-4">
        ...
        <div>
          <button
            className="w-full sm:w-auto bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600 transition duration-200"
            onClick={() => setIsQuestionBankOpen(true)} // Open QuestionBankModal
          >
