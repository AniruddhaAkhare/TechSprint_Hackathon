...
import { setDoc,getDocs, collection, addDoc, updateDoc, doc } from 'firebase/firestore';
...
const CreateSession = ({ isOpen, toggleSidebar, session }) => {
  ...
  useEffect(() => {
    if (session) {
      ...
      setSessionDetails(session.description);
      setSessionType(session.fee);
      setSelectedBatches(session.duration);
      setInstructor(session.prerequisites);
      setMode(session.mode);
      setMeetingPlatform(session.branch);
      setApprovalRequired(session.subjects);
      setShareableLink(session.subjects);
      setAllowGuests(session.subjects);
      setStartTime(session.subjects);
      setEndTime(session.subjects);
      setGenerateLink(session.subjects);
      setAllowWithoutLogin(session.subjects);
    }else{
      setSessionName("");
      setSessionDetails("");
      setSessionType("");
    ...
    <div className={`fixed top-0 right-0 h-full bg-white w-2/5 shadow-lg transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'} p-4 overflow-y-auto`}>
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-6">
        ...
        <div className="flex justify-end space-x-4 mt-4">
          ...
          {/* For future -> by clicking this button, it will send to all the studnets */}
          {/* <button
            type="button"
