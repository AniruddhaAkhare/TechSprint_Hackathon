...
//     const triggerDueReminders = async (callSchedules) => {
//         const now = Date.now();
//         for (const schedule of callSchedules) {
//           if (schedule.status !== "scheduled") continue;
//           const callDateTime = new Date(`${schedule.callDate}T${schedule.callTime}`);
//           if (isNaN(callDateTime.getTime())) {
//             console.warn("Invalid call date/time for schedule:", schedule.id);
//             continue;
//           }
//           const reminderDateTime = new Date(callDateTime.getTime() - parseInt(schedule.reminderTime) * 60000);
//           const timeDiff = reminderDateTime.getTime() - now;
      
//           if (timeDiff <= 300000 && timeDiff > -60000) { // Within 5min window or up to 1min overdue
//             try {
//               const companyRef = doc(db, "Companies", schedule.companyId);
//               const companyDoc = await getDoc(companyRef);
...
//               if (hasPermission) {
//                 new Notification("Call Reminder", {
//                   body: `Call scheduled with ${companyName} at ${schedule.callTime}: ${schedule.content}`,
//                   icon: "/path/to/icon.png",
//                 });
//               } else {
//                 console.warn("Notification permission not granted for reminder:", schedule.id);
//                 toast.warn("Notification permission not granted for call reminder.");
//               }
...
//                 purpose: schedule.content,
//               });
//               setOpenReminderDialog(true);
//               logActivity("TRIGGER_DUE_CALL_REMINDER", {
...
//               await updateDoc(doc(db, "Companies", schedule.companyId, "notes", schedule.id), {
//                 status: "notified",
//                 updatedAt: serverTimestamp(),
//               });
//             } catch (error) {
//               console.error("Error triggering due reminder:", error);
//               toast.error(`Failed to trigger reminder: ${error.message}`);
//             }
//           }
//         }
...
// //     const now = Date.now();
// //     for (const schedule of callSchedules) {
// //         if (schedule.status !== "scheduled") continue; // Skip non-scheduled calls
// //         const callDateTime = new Date(`${schedule.callDate}T${schedule.callTime}`);
// //         const reminderDateTime = new Date(callDateTime.getTime() - parseInt(schedule.reminderTime) * 60000);
// //         const timeDiff = reminderDateTime.getTime() - now;

// //         // Trigger if reminder is due (within 5 minutes) or overdue
// //         if (timeDiff <= 300000 && timeDiff > -60000) { // Within 5min window or up to 1min overdue
// //             try {
// //                 const companyRef = doc(db, "Companies", schedule.companyId);
// //                 const companyDoc = await getDoc(companyRef);
...
// // Modified useEffect for fetching call schedules
// useEffect(() => {
//     if (!selectedCompany?.id) return;
//     const fetchCallSchedules = async () => {
//         try {
...
//             );
//             const snapshot = await getDocs(q);
//             const schedules = snapshot.docs.map((doc) => ({ id: doc.id, companyId: selectedCompany.id, ...doc.data() }));
//             setCallSchedules(schedules);
//             // Check for due/overdue reminders
//             await triggerDueReminders(schedules);
//         } catch (error) {
//             console.error("Error fetching call schedules:", error);
//             if (error.code === "failed-precondition" && error.message.includes("Theontol The query requires an index")) {
//                 toast.error(
//                     "Failed to fetch call schedules: A Firestore index is required. Create it in the Firebase Console.",
//                     { autoClose: 10000 }
//                 );
...
//                     disabled={!callScheduleForm.companyId || !callScheduleForm.callDate || !callScheduleForm.callTime || !callScheduleForm.purpose}
//                   >
//                     Schedule
//                   </Button>
//                 </DialogFooter>
...
<<<<<<< HEAD
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 p-4 fixed inset-0 left-[300px]">
          ...
          <table className="w-full">
            ...
            <tbody>
              ...
              {(searchResults.length > 0 ? searchResults : companies).length === 0 && (
                <tr>
                  <td
                    colSpan={(canUpdate || canDelete || canCreate) ? 9 : 8}
        ...
        >
          ...
          <DialogBody className="text-gray-600 space-y-4">
            ...
            <Textarea
              label="Purpose"
              value={callScheduleForm.purpose}
              onChange={(e) => setCallScheduleForm(e.target, { ...e.target, purpose: e.target.value })}
            />
            <Select
              label="Reminder (minutes before)"
              value={callScheduleForm.reminderTime}
              onChange={(value) => setCallScheduleForm({ ...callScheduleForm, reminderTime: value })}
            >
              <Option value="5">5 minutes</Option>
              <Option value="15">15 minutes</Option>
              <Option value="30">30 minutes</Option>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                !callScheduleForm.companyId ||
                !callScheduleForm.callDate ||
                !callScheduleForm.callDate ||
                !callScheduleForm.purpose
              }
            >
              Schedule
            </Button>
      ...
      {openReminderDialog && reminderDetails && (
        <Dialog
          open={openCallScheduleDialog}
          dialogHandler={() => {}}
          className="max-w-sm rounded-lg shadow-lg"
        >
          <DialogHeader className="text-blue-gray-800 font-semibold">Call Reminder</DialogHeader>
        <DialogBody className="text-gray-600 space-y-2">
          <p>
            <strong>Company:</strong> {reminderDetails.companyName}
          </p>
          <p>
            <strong>Call Time:</strong> {callDate} {callTime}
          </p>
          <p>
            <strong>Purpose:</strong> {reminderDetails.purpose}
          </p>
        </DialogBody>
        <DialogFooter>
          <Button
