...
export default function Companies() {
  ...
  // Format date safely for notes
  const formatDateSafely = (date, dateFormat) => {
    if (!date) return "N/A";
    try {
      if (typeof date === "string") return date; // If it's already a formatted string, return as-is
      if (date.toDate) date = date.toDate(); // Convert Firestore Timestamp to Date
...
const logActivity = async (action, details) => {
  ...
  const fetchCompanies = async () => {
    try {
      ...
      const snapshot = await getDocs(q);
      const companyDataPromises = snapshot.docs.map(async (companyDoc) => {
        const company = {
          id: companyDoc.id,
          ...companyDoc.data(),
          pointsOfContact: Array.isArray(companyDoc.data().pointsOfContact)
            ? companyDoc.data().pointsOfContact
            : [],
        };
        ...
        return { ...company, notes: notes.slice(0, 2) }; // Store only the 2 most recent notes
      });
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gradient-to-r from-gray-100 to-gray-200">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider min-w-64">
                  Notes
                </th>
            ...
            <tbody className="divide-y divide-gray-200">
                    ...
                    {company.notes && company.notes.length > 0 ? (
                      <div className="space-y-2">
                        {company.notes.map((note, noteIndex) => (
                          <div key={note.id || noteIndex} className="border border-gray-200 rounded-md p-2 bg-gray-50">
                            <div className="flex justify-between items-center mb-1">
                              <p className="text-xs font-medium text-gray-700">
                                {formatNoteType(note.noteType)}
                              </p>
                              <p className="text-xs text-gray-500">
                                {formatDateSafely(note.createdAt, "MMM d, h:mm a")}
                              </p>
                            </div>
                            <p className="text-xs text-gray-900 truncate">{note.content}</p>
                            {note.noteType === "call-schedule" && (
                              <div className="text-xs text-gray-600 mt-1">
                                <p>Call: {note.callDate} {note.callTime}</p>
                              </div>
                            )}
                          </div>
                        ))}
              ...
              {(searchResults.length > 0 ? searchResults : companies).length === 0 && (
                <tr>
                  <td
                    colSpan={(canUpdate || canDelete || canCreate) ? 10 : 9}
              ...
              {!callScheduleForm.companyId && (
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setCallScheduleForm({ ...callScheduleForm, companyId: e.target.value })
                    }
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-sm border border-gray-100">
            ...
            <div className="space-y-2 text-sm text-gray-700">
              <p>
                ...
                {reminderDetails.companyName}
              </p>
              <p>
                <span className="font-medium text-gray-900">Call Time:</span>{" "}
                {reminderDetails.callDate} {reminderDetails.callTime}
              </p>
              <p>
                <span className="font-medium text-gray-900">Purpose:</span> {reminderDetails.purpose}
              </p>
            </div>
