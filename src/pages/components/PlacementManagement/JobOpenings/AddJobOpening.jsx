...
//         setPoc(job.poc || "");
//         setClosingDate(job.closingDate ? job.closingDate.toDate().toISOString().split("T")[0] : "");
//         setPostingDate(job.postingDate ? job.postingDate.toDate().toISOString().split("T")[0] : new Date().toISOString().split("T")[0]);
...
//     if (!title.trim() || !companyId || !jobType || !experienceMin || !experienceMax || !salary || !locationType || !poc || !closingDate || !postingDate) {
...
//     const jobStatus = closingDateObj < currentDate ? "Inactive" : "Open";
...
//       poc,
//       status: jobStatus,
//       closingDate: Timestamp.fromDate(closingDateObj),
...
const AddJobOpening = ({ isOpen, toggleSidebar, job }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const closingDateObj = new Date(closingDate);
    const postingDateObj = new Date(postingDate);
    if (isNaN(closingDateObj.getTime()) || isNaN(postingDateObj.getTime())) {
