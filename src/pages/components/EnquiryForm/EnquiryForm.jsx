...
import { getDoc, getDocs, collection, deleteDoc, doc, query, orderBy, onSnapshot, updateDoc, increment, arrayUnion } from 'firebase/firestore';
...
export default function EnquiryForms() {
    ...
    const [deleteMessage, setDeleteMessage] = useState("Are you sure you want to delete this form? This action cannot be undone.");
...
const handleViewForm = (form) => {
    ...
    console.log("User permissions:", rolePermissions); // Debug permissions
    ...
    const fetchEnquiryCounts = useCallback(async () => {
        try {
            ...
            console.log("Enquiry counts:", counts); // Debug counts
    ...
    //   useEffect(() => {
    //     const fetchEnquiryForms = async () => {
    //       try {
    //         const snapshot = await getDocs(collection(db, "EnquiryForms"));
    //         const forms = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    //         console.log("Fetched enquiry forms:", forms);
    //         setEnquiryForms(forms);
    //       } catch (error) {
                    ...
                    <table className="w-full table-auto">
                        <thead className="bg-gray-100">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-base font-semibold text-gray-700">By User/Role</th>
                        ...
                        <tbody>
                            {(searchResults.length > 0 || searchTerm.trim() ? searchResults : forms).map((form, index) => (
                                <tr key={form.id} className="border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3 text-gray-600">{form.createdBy || 'N/A'} ({form.role || 'N/A'})</td>
                                    ...
                                    <td className="px-4 py-3">
                                        <FormControl size="small">
                                            ...
                                            >
                                                ...
                                                <MenuItem value="view" onClick={() => handleViewForm(form)}>View Form</MenuItem>
            ...
            {canDelete && (
                ...
                >
                    ...
                    <DialogFooter className="space-x-4 p-4">
                        ...
                        {deleteMessage === "Are you sure you want to delete this form? This action cannot be undone." && (
                            <Button
                                variant="filled"
                                color="red"
                                onClick={deleteForm}
