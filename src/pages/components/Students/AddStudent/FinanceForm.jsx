...
//       const key = `ignitefireblaze/students/${studentId}/${docType}_${timestamp}_${file.name}`;
//       const params = {
//         Bucket: bucketName,
//         Key: key,
//         Body: file,
...
const FinanceForm = ({
      ...
      for (const docType of docTypes) {
        try {
          const command = new ListObjectsV2Command({
            Bucket: bucketName,
            Prefix: `students/${studentId}/${docType}/`,
          });
          const { Contents } = await s3Client.send(command);
          if (Contents && Contents.length > 0) {
            // Get the most recent file (assuming one file per docType folder)
            const latestFile = Contents.sort((a, b) => new Date(b.LastModified) - new Date(a.LastModified))[0];
            const s3Url = `https://${bucketName}.s3.${region}.amazonaws.com/${latestFile.Key}`;
            const fileName = latestFile.Key.split("/").pop();
            handleFinanceChange(courseIndex, docType, "", s3Url);
            handleFinanceChange(courseIndex, `${docType}Name`, "", fileName);
          }
        } catch (err) {
          console.error(`Failed to fetch ${docType} from S3:`, err);
          setAttachmentError((prev) => ({ ...prev, [docType]: `Failed to fetch ${docType}: ${err.message}` }));
        }
      }
    };

    if (bucketName && region && studentId && s3Client.config.credentials) {
      fetchDocuments();
    }
  }, [bucketName, region, studentId, courseIndex, handleFinanceChange]);
  ...
  const handleFileChangeS3 = async (courseIndex, docType, event) => {
    ...
    try {
      ...
      if (financeDetails[docType]) {
        let key = new URL(financeDetails[docType]).pathname.substring(1);
        key = decodeURIComponent(key);
      ...
      const timestamp = Date.now();
      const key = `students/${studentId}/${docType}/${docType}_${timestamp}_${file.name}`;
      const params = {
        Bucket: bucketName,
        Key: key,
        Body: file,
                ...
                <TableCell>
                  ...
                  {uploadProgress[doc.type] && uploadProgress[doc.type] !== 100 && (
                    <Typography variant="body2" className="mt-1">
                      {uploadProgress[doc.type] === -1
                        ? "Upload failed"
                        : `Upload progress: ${uploadProgress[doc.type]}%`}
                    </Typography>
                  )}
                </TableCell>
              </TableRow>
