...
const AddCourse = () => {
    ...
    const [courseEntries, setCourseEntries] = useState([]);
    const [financePartners, setFinancePartners] = useState([]);
    const [preferredCenters, setPreferredCenters] = useState([]); // State for preferred centers
    const [centers, setCenters] = useState([]); // State for center details
    const [loading, setLoading] = useState(true);
    const [isOpen, setIsOpen] = useState(false);
    const navigate = useNavigate();
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch student data to get preferred centers
                ...
                // Fetch centers to map IDs to names
                const centersSnapshot = await getDocs(collection(db, 'Centers'));
                const fetchedCenters = centersSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    ...
    // Filter courses based on preferred centers and selected mode
    const getFilteredCourses = (mode) => {
        if (!mode || preferredCenters.length === 0) return [];
    
        return courses.filter(course => {
            // Check if the course is available at any of the preferred centers (only active ones)
            const isAvailableAtCenter = course.centers?.some(
                center => center.status === "Active" && preferredCenters.includes(center.centerId)
            );
            // Check if the course mode matches the selected mode
        ...
        <>
            ...
            >
                <div className="p-6">
                    ...
                    {/* Preferred Learning Centers Section */}
                    <div className="mb-6">
                    ...
                    {courseEntries.map((entry, courseIndex) => (
                        <div key={courseIndex} className="mb-8 bg-white rounded-lg shadow-md p-6">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                ...
                                {/* Course Dropdown (Second) - Filtered by mode and preferred centers */}
                                <Select
                                    value={entry.selectedCourse || ''}
                                    onChange={(e) => handleChange(courseIndex, 'selectedCourse', e.target.value)}
                                    displayEmpty
                                    className="w-full bg-gray-100 rounded-lg"
                                    renderValue={(selected) => selected ? selected.name : <span className="text-gray-400">Select Course</span>}
                                    disabled={!entry.mode} // Disable if mode is not selected
                                >
