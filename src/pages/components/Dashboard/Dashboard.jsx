...
const Dashboard = () => {
        ...
        (snapshot) => {
          if (col === "enquiries") {
            ...
            const prequalifiedCount = enquiriesData.filter(
              (enquiry) => enquiry.status === "prequalified"
            ).length;
            setData((prev) => ({
              ...prev,
  ...
  const handlePieClick = (event, elements, type) => {
    if (elements.length > 0) {
      const index = elements[0].index;
      const statuses = type === "enquiries"
        ? ["prequalified", "qualified", "negotiation", "closed won", "closed lost", "contact in future"]
  ...
  const studentStatusCounts = getStudentStatusCounts();
  const centerCounts = getCenterCounts();
  const enquiryStatusCounts = getEnquiryStatusCounts();
  ...
  const enquiryPieData = {
    labels: ["Prequalified", "Qualified", "Negotiation", "Closed Won", "Closed Lost", "Contact in Future"],
    datasets: [
      {
        data: [
          enquiryStatusCounts.prequalified || 0,
          enquiryStatusCounts.qualified || 0,
          enquiryStatusCounts.negotiation || 0,
          enquiryStatusCounts["closed won"] || 0,
          enquiryStatusCounts["closed lost"] || 0,
          enquiryStatusCounts["contact in future"] || 0,
        ],
