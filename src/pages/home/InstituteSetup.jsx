...
//     academicYearEnd: '',
//     timezone: '',
//     logoUrl: '', // Add logoUrl to store the uploaded logo URL
//   });
//   const [logoFile, setLogoFile] = useState(null); // State to store the selected logo file
//   const [logoError, setLogoError] = useState(''); // State to store validation errors
// const [branches, setBranches] = useState([]);
//   // const [showModal, setShowModal] = useState(false);
//   const [currentBranch, setCurrentBranch] = useState(null);
//   const [branchForm, setBranchForm] = useState({
...
//     try {
//       const fileKey = `logos/${Date.now()}_${logoFile.name}`; // Unique key
//       const params = {
//         Bucket: process.env.REACT_APP_S3_BUCKET_NAME,
//         Key: fileKey,
//         Body: logoFile,
//         ContentType: logoFile.type,
//         ACL: 'public-read', // Public access (adjust if needed)
//       };

//       const command = new PutObjectCommand(params);
//       await s3Client.send(command);

//       const logoUrl = `https://${params.Bucket}.s3.${process.env.REACT_APP_AWS_REGION}.amazonaws.com/${params.Key}`;
//       setFormData((prev) => ({ ...prev, logoUrl }));
//       setLogoError('');
//       return logoUrl;
//     } catch (error) {
//       console.error('Error uploading logo to S3:', error);
//       setLogoError('Failed to upload logo. Check credentials or bucket permissions.');
//       return null;
//     }
//   };
...
//         if (!logoUrl) return; // Stop if upload fails
...
//                       <div className="logo-preview">
//                         <p>Current Logo:</p>
//                         <img src={formData.logoUrl} alt="Institute Logo" style={{ maxWidth: '200px' }} />
//                       </div>
//                     )}
//                   </div>
...
const InstituteSetup = () => {
    ...
    const fetchInstituteData = async () => {
      ...
      if (!querySnapshot.empty) {
        ...
        const branchesSnapshot = await getDocs(
          collection(db, 'instituteSetup', querySnapshot.docs[0].id, 'Centers')
        );
  ...
  const handleLogoUpload = async () => {
    ...
    try {
      const fileKey = `logos/${Date.now()}_${logoFile.name}`;
      const params = {
        Bucket: process.env.REACT_APP_S3_BUCKET_NAME,
        Key: fileKey,
        Body: logoFile,
